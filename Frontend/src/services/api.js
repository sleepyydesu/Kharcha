...
// ── Payment Sessions (per-transaction Dynamic QR) ──────────────
// Org creates a session with a fixed amount; customer scans & pays.
export const createPaymentSession = (body) =>
    request("/org/qr-codes/payments/create", {
        method: "POST",
        body: JSON.stringify(body),
    });
// Merchant polls this (every ~2 s) to detect payment automatically.
export const getPaymentSessionStatus = (session_id) =>
    request(`/org/qr-codes/payments/status/${session_id}`);
// ── POS Checkout (QR at store) ─────────────────────────────────
// Public — no auth — called by Kharcha scanner to get session details
