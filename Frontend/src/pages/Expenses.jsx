...
import {
    ...
    ChevronRight, ChevronLeft, Pencil, Trash2, Briefcase,
    Building2, Gift, MoreHorizontal, DollarSign, Zap,
    Search, ArrowUpRight, ArrowDownRight, PiggyBank,
    AlertTriangle, BarChart2, Calendar, Download,
} from "lucide-react";
import "./Expenses.css";
import CategoryIcon from "../components/CategoryIcon";
import {
    getExpenseOverview, createExpense, updateExpense, deleteExpense,
    getExpensesByCategory, getIncome, createIncome, updateIncome,
    deleteIncome, getBudgets, createBudget, updateBudget, deleteBudget,
    getCategories,
} from "../services/api";

const INCOME_SOURCES = ["Salary","Freelance","Business","Investments","Gifts","Transfer","Others"];

const CAT_COLORS = [
    "#1e5c38","#2e7d55","#4caf50","#81c784","#388e3c",
    "#66bb6a","#a5d6a7","#0d7c4a","#27ae60","#52c98c",
];
...
function AutoBadge() {
    return <span className="auto-badge"><Zap size={8} /> Auto</span>;
}
        ...
        <div className="hero-panel">
            ...
            <div className="hero-stats">
                <div className="hero-stat">
                    <div className="hero-stat-icon income-bg"><TrendingUp size={14} /></div>
                    <div>
                        <p className="hero-stat-label">Income</p>
                        <p className="hero-stat-val income-val">NPR {totalIncome.toLocaleString()}</p>
                    </div>
                </div>
                <div className="hero-stat-divider" />
                <div className="hero-stat">
                    <div className="hero-stat-icon expense-bg"><TrendingDown size={14} /></div>
                    <div>
                        <p className="hero-stat-label">Spent</p>
                        <p className="hero-stat-val expense-val">NPR {totalExpenses.toLocaleString()}</p>
                    </div>
                </div>
                <div className="hero-stat-divider" />
                <div className="hero-stat">
                    <div className="hero-stat-icon avg-bg"><Calendar size={14} /></div>
                    <div>
    ...
    return (
        <div className="top-cat-callout">
            ...
            <div className="top-cat-right">
                ...
                <span className="top-cat-amt">NPR {parseFloat(top.total_amount).toLocaleString()}</span>
            </div>
        </div>
    );
}
        ...
        <div className="chart-card">
            ...
            <ResponsiveContainer width="100%" height={150}>
                <BarChart data={data} barCategoryGap="30%" barGap={6} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                    ...
                    <YAxis hide />
                    <Tooltip formatter={(v) => `NPR ${v.toLocaleString()}`} contentStyle={{ borderRadius: 10, border: "none", boxShadow: "0 4px 20px rgba(0,0,0,.12)", fontSize: 12 }} />
...
function BudgetDonut({ totalExpenses, budgets, overview }) {
    ...
    if (!hasBudget) {
        pieData = overview.filter((c) => parseFloat(c.total_amount) > 0).map((c, i) => ({ name: c.category_name, value: parseFloat(c.total_amount), color: CAT_COLORS[i % CAT_COLORS.length] }));
        centerLabel = `NPR ${totalExpenses.toLocaleString()}`;
        centerSub = "Total Spent";
    } else if (isOverspent) {
        pieData = overview.filter((c) => parseFloat(c.total_amount) > 0).map((c, i) => ({ name: c.category_name, value: parseFloat(c.total_amount), color: `hsl(${i * 14}, 65%, ${42 + i * 4}%)` }));
        centerLabel = `-NPR ${(totalExpenses - totalBudget).toLocaleString()}`;
        centerSub = "Over Budget";
    } else {
        const expParts = overview.filter((c) => parseFloat(c.total_amount) > 0).map((c, i) => ({ name: c.category_name, value: parseFloat(c.total_amount), color: CAT_COLORS[i % CAT_COLORS.length] }));
    ...
    if (pieData.length === 0) { pieData = [{ name: "No Data", value: 1, color: "#e2e8f0" }]; centerLabel = "NPR 0"; centerSub = "No expenses"; }
        ...
        <div className="chart-card">
            ...
            <div className="budget-donut-wrap">
                <ResponsiveContainer width="100%" height={160}>
                    <PieChart>
                        <Pie data={pieData} dataKey="value" cx="50%" cy="50%" innerRadius={48} outerRadius={72} startAngle={90} endAngle={-270} strokeWidth={2} stroke="white">
        ...
        <div className="cat-row" onClick={onClick}>
            <div className="cat-row-left">
                <div className="cat-row-icon-wrap" style={{ background: `${CAT_COLORS[index % CAT_COLORS.length]}18` }}>
                    <CategoryIcon iconUrl={cat.icon_url} iconType={cat.icon_type} name={cat.category_name} color={CAT_COLORS[index % CAT_COLORS.length]} size={28} />
                </div>
        ...
        <div className="cat-row" onClick={onClick}>
            <div className="cat-row-left">
                ...
                <div className="cat-row-info">
                    ...
                    <span className="cat-row-count">{count} entr{count !== 1 ? "ies" : "y"}</span>
                </div>
            </div>
            <div className="cat-row-right">
                <span className="cat-row-amount income-amount">+NPR {total.toLocaleString()}</span>
                <ChevronRight size={16} className="cat-row-arrow" />
            </div>
        </div>
    );
...
function FormField({ label, children, required }) {
    return (
        <div className="exp-form-group">
            <label className="exp-form-label">{label}{required && <span className="required-star"> *</span>}</label>
            {children}
        </div>
    );
}
...
function EditExpenseSheet({ exp, categories, open, onClose, onSaved }) {
    const [form, setForm] = useState({ amount: "", note: "", date: "", category_id: "" });
    const [saving, setSaving] = useState(false);
    const [err, setErr] = useState("");

    useEffect(() => {
        if (exp) { setForm({ amount: exp.amount, note: exp.note || "", date: exp.date, category_id: exp.category_id || "" }); setErr(""); }
    }, [exp]);
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            await updateExpense(exp.expense_id, { category_id: form.category_id || exp.category_id, amount: parseFloat(form.amount), note: form.note || null, date: form.date });
            onSaved(); onClose();
    ...
    return (
        <BottomSheet open={open} onClose={onClose}>
            <div className="exp-modal-header">
                ...
                <button className="exp-modal-close" onClick={onClose}><X size={20} /></button>
            </div>
            {err && <p className="exp-modal-err">{err}</p>}
            <div className="exp-modal-body">
                <FormField label="Category" required>
                    <select className="exp-input" value={form.category_id} onChange={(e) => setForm({ ...form, category_id: e.target.value })}>
                        {categories.map((c) => <option key={c.category_id} value={c.category_id}>{c.name}</option>)}
                    </select>
                </FormField>
                <FormField label="Amount (NPR)" required>
                    <input type="number" className="exp-input" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder="0.00" />
                </FormField>
                <FormField label="Date" required>
                    <input type="date" className="exp-input" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} />
                </FormField>
                <FormField label="Note (optional)">
                    <input type="text" className="exp-input" value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} placeholder="Add a note..." maxLength={100} />
                </FormField>
                <button className="exp-submit-btn expense" onClick={handleSave} disabled={saving}>{saving ? "Saving..." : "Save Changes"}</button>
            </div>
        </BottomSheet>
    );
}
...
function EditIncomeSheet({ inc, open, onClose, onSaved }) {
    const [form, setForm] = useState({ amount: "", source: "Salary", note: "", date: "" });
    const [saving, setSaving] = useState(false);
    const [err, setErr] = useState("");

    useEffect(() => {
        if (inc) { setForm({ amount: inc.amount, source: inc.source || "Others", note: inc.note || "", date: inc.date }); setErr(""); }
    }, [inc]);
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            await updateIncome(inc.income_id, { amount: parseFloat(form.amount), source: form.source, note: form.note || null, date: form.date });
            onSaved(); onClose();
        } catch (e) { setErr(e.message || "Failed to save."); }
        setSaving(false);
    };
        ...
        <BottomSheet open={open} onClose={onClose}>
            ...
            <div className="exp-modal-body">
                <FormField label="Source" required>
                    <select className="exp-input" value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })}>
                ...
                <button className="exp-submit-btn income" onClick={handleSave} disabled={saving}>{saving ? "Saving..." : "Save Changes"}</button>
            </div>
        </BottomSheet>
    );
}
...
function CategoryDetailView({ cat, range, categories, onBack, onRefresh }) {
    ...
    const filtered = useMemo(() => {
        ...
        return expenses.filter((e) => (e.note || "").toLowerCase().includes(q) || e.date.includes(q));
    }, [expenses, search]);

    const exportCSV = () => {
        const rows = [["Date","Note","Amount"], ...expenses.map((e) => [e.date, e.note || "", e.amount])];
    ...
    return (
        <div className="cat-detail-view">
            <div className="cat-detail-header">
                ...
                <div className="cat-detail-title">
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <button className="icon-btn" onClick={exportCSV} title="Export CSV"><Download size={15} /></button>
                    </div>
                </div>
            </div>
            <div className="detail-search-wrap">
                <Search size={14} className="detail-search-icon" />
                <input type="text" className="detail-search-input" placeholder="Search by note or date..." value={search} onChange={(e) => setSearch(e.target.value)} />
                {search && <button className="detail-search-clear" onClick={() => setSearch("")}><X size={13} /></button>}
            </div>
            {loading ? (
                <div className="skeleton-list">{[1,2,3].map((i) => <SkeletonCard key={i} />)}</div>
            ) : filtered.length === 0 ? (
                <div className="exp-empty">
                    <PiggyBank size={36} strokeWidth={1.2} />
                    <p>{search ? "No results match your search." : "No expenses in this category."}</p>
                </div>
            ) : (
                    ...
                    {filtered.map((exp) => (
                        <div key={exp.expense_id} className="stmt-row">
                            <div className="stmt-row-left">
                                ...
                                <div>
                                    <p className="stmt-row-name">{exp.note || exp.categories?.name || cat.category_name || "Expense"}{exp.is_auto && <AutoBadge />}</p>
                            ...
                            <div className="stmt-row-right">
                                ...
                                <div className="row-actions">
                                    <button className="row-action-btn edit" onClick={() => { setEditingExp(exp); setEditSheetOpen(true); }}><Pencil size={13} /></button>
            ...
            <EditExpenseSheet exp={editingExp} categories={categories} open={editSheetOpen} onClose={() => setEditSheetOpen(false)} onSaved={async () => { await load(); onRefresh(); }} />
        </div>
    );
}
...
function IncomeSourceDetailView({ source, entries, onBack, onRefresh }) {
    ...
    const handleDelete = async (id) => {
        ...
        try { await deleteIncome(id); setLocalEntries((prev) => prev.filter((e) => e.income_id !== id)); onRefresh(); }
        catch (e) { console.error(e); }
    };
    ...
    const filtered = useMemo(() => {
        ...
        const q = search.toLowerCase();
        return localEntries.filter((e) => (e.note || "").toLowerCase().includes(q) || e.date.includes(q));
    }, [localEntries, search]);
    ...
    return (
        <div className="cat-detail-view">
            <div className="cat-detail-header">
                ...
                <div className="cat-detail-title">
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div className="cat-row-icon-wrap" style={{ background: cfg.bg, width: 32, height: 32 }}>
                            <Icon size={16} color={cfg.color} strokeWidth={2.2} />
                        </div>
                        <span>{source}</span>
                    </div>
                    <span className="cat-detail-total" style={{ color: "#16a34a" }}>+NPR {total.toLocaleString()}</span>
                </div>
            </div>
            <div className="detail-search-wrap">
                <Search size={14} className="detail-search-icon" />
            ...
            {filtered.length === 0 ? (
                <div className="exp-empty">
                    ...
                    <p>{search ? "No results match your search." : "No income entries for this source."}</p>
                </div>
            ) : (
                <div className="stmt-list">
                    {filtered.map((inc) => (
                        <div key={inc.income_id} className="stmt-row">
                            <div className="stmt-row-left">
                                ...
                                <div>
                                    <p className="stmt-row-name">{inc.note || source}{inc.is_auto && <AutoBadge />}</p>
                                    <p className="stmt-row-meta">{inc.date}</p>
                                </div>
                            </div>
                            <div className="stmt-row-right">
                                <span className="stmt-row-amount" style={{ color: "#16a34a" }}>+NPR {parseFloat(inc.amount).toLocaleString()}</span>
                                <div className="row-actions">
                                    <button className="row-action-btn edit" onClick={() => { setEditingInc(inc); setEditSheetOpen(true); }}><Pencil size={13} /></button>
                                    <button className="row-action-btn delete" onClick={() => handleDelete(inc.income_id)}><Trash2 size={13} /></button>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            <EditIncomeSheet inc={editingInc} open={editSheetOpen} onClose={() => setEditSheetOpen(false)} onSaved={() => { onRefresh(); onBack(); }} />
        </div>
    );
}
...
function BudgetCard({ budget: b, categories, onRefresh }) {
    ...
    const [saving, setSaving] = useState(false);
    const [err, setErr] = useState("");
    const [form, setForm] = useState({ amount: b.amount, category_id: b.category_id || "", period_start: b.period_start, period_end: b.period_end });

    const handleDelete = async () => {
        if (!window.confirm("Delete this budget?")) return;
        try { await deleteBudget(b.budget_id); onRefresh(); } catch (e) { console.error(e); }
    };
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            await updateBudget(b.budget_id, { amount: parseFloat(form.amount), category_id: form.category_id || null, period_start: form.period_start, period_end: form.period_end });
            setEditing(false); onRefresh();
        } catch (e) { setErr(e.message || "Failed to save."); }
        setSaving(false);
    };

    const barColor = b.utilization_pct >= 90 ? "#ef4444" : b.utilization_pct >= 70 ? "#f59e0b" : "#1e5c38";

    return (
        <div className="exp-budget-card">
            {editing ? (
                <div className="budget-edit-form">
                    {err && <p className="exp-modal-err" style={{ marginBottom: 10 }}>{err}</p>}
                    <FormField label="Category">
                        <select className="exp-input" style={{ padding: "8px 12px", fontSize: 13 }} value={form.category_id} onChange={(e) => setForm({ ...form, category_id: e.target.value })}>
                            <option value="">Overall Budget</option>
                            {categories.map((c) => <option key={c.category_id} value={c.category_id}>{c.name}</option>)}
                        </select>
                    </FormField>
                    <FormField label="Amount (NPR)" required>
                        <input type="number" className="exp-input" style={{ padding: "8px 12px", fontSize: 13 }} value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} />
                    </FormField>
                    <div style={{ display: "flex", gap: 8 }}>
                        <div className="exp-form-group" style={{ flex: 1 }}>
                            <label className="exp-form-label">Start</label>
                            <input type="date" className="exp-input" style={{ padding: "8px 12px", fontSize: 13 }} value={form.period_start} onChange={(e) => setForm({ ...form, period_start: e.target.value })} />
                        </div>
                        <div className="exp-form-group" style={{ flex: 1 }}>
                            <label className="exp-form-label">End</label>
                            <input type="date" className="exp-input" style={{ padding: "8px 12px", fontSize: 13 }} value={form.period_end} onChange={(e) => setForm({ ...form, period_end: e.target.value })} />
                        </div>
                    </div>
                    <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                        <button className="edit-save-btn" onClick={handleSave} disabled={saving} style={{ flex: 1 }}>{saving ? "Saving..." : "Save"}</button>
                        <button className="edit-cancel-btn" onClick={() => { setEditing(false); setErr(""); }} style={{ flex: 1 }}>Cancel</button>
                    </div>
                </div>
            ) : (
                <>
                    <div className="exp-budget-top">
                        <div>
                            ...
                            <p className="exp-budget-meta">
                                NPR {parseFloat(b.spent || 0).toLocaleString()} of NPR {parseFloat(b.amount).toLocaleString()}
                                <span className="exp-budget-period"> · {b.period_start} → {b.period_end}</span>
                            </p>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <span className={`exp-budget-pct ${b.utilization_pct >= 90 ? "danger" : b.utilization_pct >= 70 ? "warn" : ""}`}>{b.utilization_pct}%</span>
                            <button className="row-action-btn edit" onClick={() => setEditing(true)}><Pencil size={13} /></button>
                            <button className="row-action-btn delete" onClick={handleDelete}><Trash2 size={13} /></button>
                        </div>
                    </div>
                    <div className="exp-budget-bar-bg">
                        <div className="exp-budget-bar-fill" style={{ width: `${Math.min(b.utilization_pct, 100)}%`, background: barColor }} />
                    </div>
                    {b.utilization_pct >= 90 && (
                        <p className="budget-warning">
                            <AlertTriangle size={12} />
                            {b.utilization_pct >= 100 ? "Over budget!" : "Almost at budget limit"}
                        </p>
                    )}
                </>
            )}
...
export default function Expenses() {
    ...
    const fetchAll = useCallback(async () => {
        ...
        try {
            ...
            const catMap = {};
            catsArr.forEach((c) => { catMap[c.category_id] = c; });
    ...
    const incomeSources = Object.values(incomeBySource).sort((a, b) => b.total - a.total);

    const openModal = (type) => { setApiErr(""); setModal(type); };
    ...
    const handleAddExpense = async () => {
        ...
        setSaving(true);
        try {
            await createExpense({ category_id: expForm.category_id, amount: parseFloat(expForm.amount), note: expForm.note || null, date: expForm.date, is_auto: false });
            setModal(null);
    ...
    const handleAddIncome = async () => {
        ...
        setSaving(true);
        try {
            await createIncome({ amount: parseFloat(incForm.amount), source: incForm.source, note: incForm.note || null, date: incForm.date, is_auto: false });
            setModal(null);
    ...
    const handleSetBudget = async () => {
        ...
        try {
            ...
            setModal(null);
            setBudForm({ category_id: "", amount: "", period_start: todayStr(), period_end: todayStr() });
            fetchAll();
    ...
    if (selectedSource && range) {
        return (
            <div className="expenses-page">
                <IncomeSourceDetailView source={selectedSource} entries={incomeBySource[selectedSource]?.entries || []} onBack={() => setSelectedSource(null)} onRefresh={fetchAll} />
            </div>
        );
    }
    if (selectedCat && range) {
        return (
            <div className="expenses-page">
                <CategoryDetailView cat={selectedCat} range={range} categories={categories} onBack={() => setSelectedCat(null)} onRefresh={fetchAll} />
            </div>
        );
    }
    ...
    return (
        <div className="expenses-page">
            ...
            {loading ? (
                ...
                <>
                    <HeroPanel totalIncome={totalIncome} totalExpenses={totalExpenses} netSavings={netSavings} range={range} />
                    {totalExpenses > 0 && <TopCategoryCallout overview={overview} totalExpenses={totalExpenses} />}
            ...
            {budgets.length > 0 && (
                <div className="exp-budget-section">
                    <h3 className="exp-section-heading"><Target size={15} /> Budgets</h3>
                    {budgets.map((b) => <BudgetCard key={b.budget_id} budget={b} categories={categories} onRefresh={fetchAll} />)}
                </div>
            )}

            {/* Expenses List */}
            <div className="exp-list-section">
                <h3 className="exp-section-heading"><TrendingDown size={15} /> Expenses by Category</h3>
                {loading ? (
                    <div className="skeleton-list">{[1,2,3].map((i) => <SkeletonCard key={i} />)}</div>
                    ...
                    <div className="exp-empty">
                        ...
                        <button className="exp-empty-cta expense" onClick={() => openModal("expense")}><Plus size={14} /> Add Expense</button>
                    </div>
                ) : (
            ...
            <div className="exp-list-section income-section">
                <h3 className="exp-section-heading income-heading"><TrendingUp size={15} /> Income by Source</h3>
                {loading ? (
                    <div className="skeleton-list">{[1,2].map((i) => <SkeletonCard key={i} />)}</div>
                ) : incomeSources.length === 0 ? (
                    <div className="exp-empty">
                        <TrendingUp size={40} strokeWidth={1.2} />
                        <p>No income recorded. Add income to track your earnings.</p>
                        <button className="exp-empty-cta income" onClick={() => openModal("income")}><Plus size={14} /> Add Income</button>
                    </div>
                ) : (
                    <div className="cat-list">
                        {incomeSources.map((src) => (
                            <IncomeSourceRow key={src.source} source={src.source} total={src.total} count={src.count} onClick={() => setSelectedSource(src.source)} />
                        ))}
                    </div>
                )}
            </div>
