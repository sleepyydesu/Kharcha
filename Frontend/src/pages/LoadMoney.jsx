...
function KhaltiBanner({ params, onDismiss }) {
  ...
  if (status === "success") {
    ...
    return (
      <div className="lm__banner lm__banner--success">
        ...
        <div>
          <strong>NPR {Number(amount).toLocaleString()} loaded!</strong>
          <p>New balance: NPR {Number(balance).toLocaleString("ne-NP", { minimumFractionDigits: 2 })}</p>
        </div>
        <button className="lm__banner-close" onClick={onDismiss}>✕</button>
      </div>
    );
  }
  if (status === "already") {
    return (
      <div className="lm__banner lm__banner--info">
        ...
        <div><strong>Already processed</strong><p>This payment was already credited to your wallet.</p></div>
        <button className="lm__banner-close" onClick={onDismiss}>✕</button>
      </div>
    );
  }
  ...
  return (
    <div className="lm__banner lm__banner--error">
      <span className="lm__banner-icon">✕</span>
      <div><strong>Payment failed</strong><p>{message}</p></div>
      <button className="lm__banner-close" onClick={onDismiss}>✕</button>
    </div>
  );
}
...
function ChooseView({ onSelect }) {
  return (
    <div className="lm__choose">
      ...
      <p className="lm__sub">Choose how you'd like to add money to your Kharcha wallet.</p>

      <div className="lm__methods">
        <button className="lm__method-card" onClick={() => onSelect("khalti")}>
          <div className="lm__method-icon lm__method-icon--khalti">
            <img src={khaltiIcon} alt="Khalti" />
          </div>
          <div className="lm__method-info">
            <span className="lm__method-name">Load with Khalti</span>
            <span className="lm__method-desc">Pay instantly via your Khalti wallet or card</span>
          </div>
          <span className="lm__method-arrow">›</span>
        </button>

        <button className="lm__method-card" onClick={() => onSelect("giftcard")}>
          ...
          <div className="lm__method-info">
            ...
            <span className="lm__method-desc">Redeem a Kharcha gift card code</span>
          </div>
          <span className="lm__method-arrow">›</span>
        </button>
      </div>
...
function KhaltiView() {
  ...
  async function handlePay() {
    ...
    if (!amt || amt < 10)  { setError("Minimum amount is NPR 10"); return; }
    if (amt > 100000)      { setError("Maximum is NPR 1,00,000"); return; }
  ...
  return (
    <div className="lm__form-wrap">
      <div className="lm__form-header lm__form-header--khalti">
        <img src={khaltiIcon} alt="Khalti" className="lm__form-header-img" />
        <div>
          <h2 className="lm__form-title">Load with Khalti</h2>
          <p className="lm__form-sub">Enter the amount you want to load</p>
        </div>
      </div>
      ...
      <div className="lm__input-row">
        ...
        <input className="lm__input" type="number" min="10" max="100000"
      ...
      <div className="lm__presets">
        {PRESETS.map(p => (
          <button key={p} className={`lm__preset ${amount == p ? "lm__preset--active" : ""}`}
      ...
      <button className="lm__pay-btn lm__pay-btn--khalti"
        onClick={handlePay} disabled={loading || !amount}>
        {loading ? "Redirecting…" : "Pay with Khalti"}
      </button>
      <p className="lm__note">You'll be redirected to Khalti to complete payment</p>
    </div>
  );
}
  ...
  if (success) {
    return (
      <div className="lm__success">
        ...
        <button className="lm__pay-btn lm__pay-btn--gift" onClick={onSuccess}>Done</button>
      </div>
    );
  }
  ...
  return (
    <div className="lm__form-wrap">
      <div className="lm__form-header lm__form-header--gift">
        <img src={giftcardIcon} alt="Gift Card" className="lm__form-header-img" />
        <div>
          <h2 className="lm__form-title">Gift Cards</h2>
          <p className="lm__form-sub">Enter your gift card code below</p>
        </div>
      </div>

      <label className="lm__label">Gift Card Code</label>
      <input className="lm__input lm__input--code" type="text"
        placeholder="XXXX-XXXX-XXXX" value={code} maxLength={20}
        onChange={e => { setCode(e.target.value.toUpperCase()); setError(""); }} />

      {error && <p className="lm__error">{error}</p>}

      <button className="lm__pay-btn lm__pay-btn--gift"
        onClick={handleRedeem} disabled={loading || !code}>
...
export default function LoadMoney() {
  ...
  // If Khalti redirected back with ?khalti=... params, show banner
  const hasKhaltiResult = searchParams.get("khalti") !== null;

  function dismissBanner() {
    // Strip the khalti params from URL without navigating
    setSearchParams({});
  }
  ...
  return (
    <div className="lm">
      {/* Khalti result banner (shown after redirect back from Khalti) */}
      {hasKhaltiResult && (
      ...
      {view === "giftcard" && <GiftView onSuccess={() => setView("choose")} />}
    </div>
  );
}