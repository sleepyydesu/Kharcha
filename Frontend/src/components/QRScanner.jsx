...
function parseQR(text) {
    try {
        ...
        // Payment session QR — generated by createPaymentSession (dynamic, amount-specific)
        if (obj.kharcha_session_id)
            return {
                type: "payment_session",
                session_id: obj.kharcha_session_id,
            };

        // Static org QR — generated from an API key, no fixed amount
        if (obj.kharcha_qr_id)
            return { type: "dynamic_qr", qr_id: obj.kharcha_qr_id };

        // Personal QR — kharcha_id is a phone number
        if (obj.kharcha_id)
            return {
                type: "payment",
                id: obj.kharcha_id,
...
export default function QRScanner({ open, onClose }) {
    ...
    const handleDetect = useCallback(
        async (raw, resume) => {
            ...
            // ── Payment session QR (kharcha_session_id) ───────────────────
            // Generated by createPaymentSession — has a fixed amount and expiry.
            if (parsed.type === "payment_session") {
                setProcessing({ label: "Reading payment QR…" });
                try {
                    const data = await resolveQRCode(parsed.session_id);
                    const qr = data.qr;
                    ...
                    setProcessing({
                        label: e.message || "QR not found or expired.",
                    });
            ...
            // ── Static org QR (kharcha_qr_id / api key) ───────────────────
            // Resolve from server to get merchant details and optional default category.
            if (parsed.type === "dynamic_qr") {
                ...
                try {
                    ...
                    if (qr.default_category)
                        params.set(
                            "default_category_id",
                            String(qr.default_category.category_id),
                        );
                    if (qr.default_category)
                        params.set(
                            "default_category_name",
                            qr.default_category.name,
                        );
                    onClose();
                    navigate(`/send?${params.toString()}`);
                } catch (e) {
