...
const validateBudgetPeriod = (period_start, period_end) => {
    ...
    if (days > MAX_BUDGET_DAYS)
        return `Budget period cannot exceed ${MAX_BUDGET_DAYS} days.`;
    return null;
};
...
const getBudgets = async (req, res) => {
    try {
        ...
        // Overlap: budget overlaps the window if period_end >= start_date AND period_start <= end_date
        if (start_date) query = query.gte("period_end", start_date);
...
const createBudget = async (req, res) => {
    try {
        ...
        if (periodErr)
            return res.status(400).json({ success: false, message: periodErr });
    ...
    try {
        ...
        if (period_start || period_end) {
            ...
            if (periodErr)
                return res
                    .status(400)
                    .json({ success: false, message: periodErr });
...
const deleteBudget = async (req, res) => {
    try {
        ...
        return res
            .status(200)
            .json({ success: true, message: "Budget deleted." });
    } catch (err) {
