...
const getProfile = async (req, res) => {
  try {
    ...
    const { data: account, error: accountError } = await supabase
      .from("accounts")
      .select(
        "account_id, account_type, email, phone_number, is_verified, is_active, kyc_status, created_at, profile_picture_url",
      )
...
const uploadProfilePicture = async (req, res) => {
  try {
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from(PROFILE_PICTURE_BUCKET).getPublicUrl(storagePath);
